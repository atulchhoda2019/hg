...
from app.state import BookingProposal, EnvelopeItem, Intent, PlanDecision, TurnState
...
def route_after_gate_output(state: TurnState) -> str:
    ...
    if state.plan.posture in ("WRITE", "DRAFT"):
        return "corridor_propose"     # a draft brand still gets the typed, checked proposal
    return "respond"
...
def route_after_propose(state: TurnState) -> str:
    if state.proposal is None:          # validation failed or a slot is missing
        return "respond"
    # Rung 1 hands the filled-in booking to the brand's own page and stops there.
    return "respond" if state.plan.posture == "DRAFT" else "corridor_preview"
...
def route_after_revalidate(state: TurnState) -> str:
    # A moved rate or a changed policy rebuilds the proposal and asks again. It never books.
    if state.proposal is not None and state.retries.get("rate_refresh", 0) == 1 \
...
def build_graph(checkpoint_path: str | None = "checkpoints.sqlite"):
    ...
    g.add_conditional_edges("corridor_wait_confirmation", corridor.route_after_confirmation,
                            {"corridor_revalidate": "corridor_revalidate",
                             "respond": "respond"})
    g.add_conditional_edges("corridor_revalidate", route_after_revalidate,
                            {"corridor_execute": "corridor_execute",
...
def serde():
    """Strict checkpoint deserialization: only this app's own state types are reconstructed."""
    return JsonPlusSerializer(allowed_msgpack_modules=[
        Intent, PlanDecision, EnvelopeItem, BookingProposal, TurnState,
    ])
